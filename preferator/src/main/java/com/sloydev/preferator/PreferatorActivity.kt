...
class PreferatorActivity : AppCompatActivity() {
    ...
    private fun addSection(
        ...
        val sectionView =
            LayoutInflater.from(this).inflate(R.layout.item_section, sectionsView, false)
        val sectionNameContainer = sectionView.findViewById<ViewGroup>(R.id.section_name_container)
        val sectionNameView: TextView = sectionView.findViewById(R.id.section_name)
        val sectionArrowView: ImageView = sectionView.findViewById(R.id.section_arrow)
        val itemsView: ViewGroup = sectionView.findViewById(R.id.section_items)
        ...
        for (pref in entries) {
            ...
            val itemView =
                LayoutInflater.from(this).inflate(R.layout.item_preference, itemsView, false)
            ...
            val editorContainer: ViewGroup = itemView.findViewById(R.id.pref_value_editor_container)
    ...
    private fun createStringEditorView(
        ...
        stringEditor.onStringValueChangeListener =
            { newValue -> preferences.edit().putString(prefKey, newValue).apply() }
        return stringEditor
    }

    private fun createIntEditorView(
        preferences: SharedPreferences,
        prefKey: String,
        prefValue: Int?
    ): IntPrefEditor {
        ...
        intEditor.onIntValueChangeListener =
            { newValue -> preferences.edit().putInt(prefKey, newValue).apply() }
        return intEditor
    }

    private fun createLongEditorView(
        preferences: SharedPreferences,
        prefKey: String,
        prefValue: Long?
    ): LongPrefEditor {
        ...
        longEditor.onLongValueChangeListener =
            { newValue -> preferences.edit().putLong(prefKey, newValue!!).apply() }
        return longEditor
    }
    ...
    private fun createFloatEditorView(
        ...
        floatEditor.onFloatValueChangeListener =
            { newValue -> preferences.edit().putFloat(prefKey, newValue).apply() }
        return floatEditor
    }

    private fun createBooleanEditorView(
        preferences: SharedPreferences,
        prefKey: String,
        prefValue: Boolean?
    ): BooleanPrefEditor {
        ...
        booleanEditor.onBooleanValueChangeListener =
            { newValue -> preferences.edit().putBoolean(prefKey, newValue).apply() }
        return booleanEditor
    }

    private fun createSetEditorView(
        preferences: SharedPreferences,
        prefKey: String,
        prefValue: Set<String>
    ): SetPrefEditor {
        val booleanEditor = SetPrefEditor(this)
        booleanEditor.value = prefValue
        booleanEditor.onSetValueChangeListener =
            { newValue -> preferences.edit().putStringSet(prefKey, newValue).apply() }
        return booleanEditor
    }
    ...
    private enum class Type(val typeName: String) {
        BOOLEAN("boolean"),
        INT("int"),
        LONG("long"),
        FLOAT("float"),
